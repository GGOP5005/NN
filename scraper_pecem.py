...
def limpar_conteineres_extras(page, container_alvo):
    """
    BALA DE PRATA CORRIGIDA: Olha o valor (value) dentro do input de cada linha.
    """
    ...
    try:
        page.evaluate(f"""
            ...
            botoes.forEach(btn => {{
                ...
                // Sobe na árvore HTML até encontrar a div/tr que abriga ESTE botão e UM ÚNICO input de contêiner
                while (parent && parent.tagName !== 'BODY') {{
                    let inputs = parent.querySelectorAll("input[name='containerNbr']");
                    if (inputs.length === 1) {{
                        meuInput = inputs[0]; // Achamos o input exato da mesma linha do botão!
                        break;
                    }} else if (inputs.length > 1) {{
                        break; // Subiu demais na árvore (pegou a tabela inteira)
                    }}
                ...
                if (meuInput) {{
                    ...
                    if (!valor.includes(alvo)) {{
                        btn.click(); // O input desta linha não é o alvo, remove!
                    }}
                }} else {{
                    // Se a linha for defeituosa e não tiver input, remove por segurança
                    btn.click();
                }}
            }});
        """)
        time.sleep(3) # Tempo para a animação do Angular remover as linhas visualmente
                    ...
                    try:
                        ...
                        if "FINALIZADO" in texto_linha:
                            ...
                            for i in range(qtd_botoes):
                                print(f"   📄 Baixando e Lendo Documento {i+1} de {qtd_botoes}...")
                                try:
                                    with aba_atend.expect_download(timeout=15000) as download_info:
                                        aba_atend.locator(f"#botao-baixar-pdf-{i}").click()
                                        
                                    download = download_info.value
                                    caminho_pdf_baixado = download.path()
                                    
                                    leitor_pdf = PyPDF2.PdfReader(caminho_pdf_baixado)
                                    texto_pdf = ""
                                    for num_pagina in range(len(leitor_pdf.pages)):
                                        texto_pdf += leitor_pdf.pages[num_pagina].extract_text() + " "
                                    
                                    texto_limpo = re.sub(r'\s+', ' ', texto_pdf)
                                    match = re.search(r"EXPIRA[CÇ][AÃ]O\s*:\s*(\d{2}/\d{2}/\d{4})", texto_limpo, re.IGNORECASE)
                                    ...
                                    print(f"   ⚠️ Não foi possível analisar o doc {i+1}. Tentando o próximo...")
